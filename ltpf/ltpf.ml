...
let state : t = Array.make 3 0;;

Array.set state 2 45;;

state;;(* 1.1 DEFINITION ET ANALYSE D'UN LANGAGE DE PROGRAMMATION SIMPLE *)
...
let (-->) (a: 'res ana) (b: 'res ana) = fun l ->
	let l = a l in b l;;

let (-|) (a: 'res ana) (b: 'res ana) = fun l ->
	try a l with Echec -> b l;;


(* pour consommer 'x' la variable *)
let p_variable : variable ana = fun l ->
  match l with
  | Var a -> l
  | _ -> raise Echec
...
let p_constante_comb : constante ana = p_zero -| p_one
  
(* pour consommer soit 0 ou soit 1 *)
let p_constante: constante ana = fun l ->
  try p_zero l with
    Echec -> p_one l;;

let p_V : expression ana = fun l ->
	match l with 
	| V var -> V( p_variable var )
	| _ -> raise Echec;;

let p_C : expression ana = fun l -> 
	match l with
	| C cons -> C ( p_constante cons )
	| _ -> raise Echec;;

let p_expression_com : expression ana = p_V -| p_C ;;
...
let p_epsilon : 'res ana = fun l -> l

let p_saxiome1: saxiome ana = fun l -> match l with 
		| Axiome ( instr, list_instr )  -> Axiome ( instr, list_instr )			
		| _ -> raise Echec

let p_liste_instr1 : liste_inst ana = fun l -> match l with
		| Plus saxiome1 -> Plus saxiome1
		| _ -> raise Echec

let p_liste_instr_Axiome: 'res ana = fun l -> 
	match l with
	| Axiome ( instr, list_instr ) -> Axiome ( instr, p_liste_instr1 list_instr )
	| _ -> raise Echec

let rec p_saxiome_com : saxiome ana = fun l -> l |> ( p_saxiome1  --> p_liste_instr_Axiome -| p_epsilon ) ;;

let rec p_liste_inst : liste_inst ana = fun l -> l |> ( p_liste_instr1 -| p_epsilon );;

let rec p_instruction : instruction ana = fun l -> 
...
let init_state :  int * int -> int array = fun (n,vaL) -> Array.make n vaL;;

let state = init_state (2,3);;
state ;;
Array.length state ;;

let get_var_i : int * int array -> int = fun ( n, l ) -> 
	if ( Array.length l - 1 ) >= n then Array.get l n else raise Echec;;

get_var_i ( 1,state );;

let rec update_var_i : int * int * int array -> unit = fun ( n, newVal, l ) ->
	if ( Array.length l - 1 ) >= n then Array.set l n newVal else raise Echec;;


let state2 = init_state (5,3);;
update_var_i ( 2, 5, state );;
